...
class _KardexPageState extends State<KardexPage> {
  ...
  Future<void> _load() async {
    if (_almacen == null || _articulo == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Seleccione almacén y artículo')));
      return;
    }
    setState(() { _loading = true; _error = null; });
    final repo = getIt<MovimientoRepository>();
    final result = await repo.getKardex(codigoAlmacen: _almacen!, codigoArticulo: _articulo!);
    result.fold(
          ...
          if (_items != null && _items!.isNotEmpty)
            IconButton(
              ...
              onPressed: () => ExportService.showExportDialog(
                ...
                columns: const ['Fecha', 'Doc', 'Tipo', 'E.Cant', 'E.Precio', 'S.Cant', 'S.Precio', 'Stock', 'Costo'],
                rows: _items!.map((k) => [
      ...
      body: Column(
        children: [
            ...
            child: Row(children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(_almacenNombre ?? 'Almacén', overflow: TextOverflow.ellipsis),
              ...
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(_articuloNombre ?? 'Artículo', overflow: TextOverflow.ellipsis),
          ...
          if (_items != null && !_loading)
            Expanded(child: _KardexTable(items: _items!)),
        ],
      ),
    );
  }
...
class _KardexTable extends StatelessWidget {
  ...
  const _KardexTable({required this.items});
      ...
      child: SingleChildScrollView(
        child: DataTable(
          ...
          columns: const [
            ...
            DataColumn(label: Text('E.Cant'), numeric: true),
            DataColumn(label: Text('E.Precio'), numeric: true),
            DataColumn(label: Text('S.Cant'), numeric: true),
            DataColumn(label: Text('S.Precio'), numeric: true),
