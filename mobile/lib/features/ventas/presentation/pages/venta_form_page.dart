...
class _Linea {
  ...
  double precio;
  double descPct;
  _Linea({required this.codigo, required this.descripcion, required this.cantidad, required this.precio, this.descPct = 0});
...
class _FormState extends State<_Form> {
  ...
  int _plazo        = 30;
  String? _almacen, _almNombre, _cliente, _cliNombre;
  ...
  Future<void> _pickCliente() async {
    ...
    if (r != null) setState(() { _cliente = r.codigo; _cliNombre = r.razonSocial; });
  }
  ...
  Future<void> _addLinea() async {
    ...
    final pCtrl   = TextEditingController(text: art.precioVenta.toStringAsFixed(4));
    ...
    final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 8),
        TextFormField(controller: dscCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Descuento %')),
      ]),
      actions: [
    ...
    if (ok == true) setState(() => _lineas.add(_Linea(
  ...
  void _submit() {
    ...
    if (_almacen == null || _cliente == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Seleccione almacén y cliente')));
      return;
    }
    if (_lineas.isEmpty) {
      ...
      body: BlocConsumer<VentaBloc, VentaState>(
        listener: (ctx, state) {
          if (state is VentaSaved) {
            ...
            ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text('Venta registrada · $nro'), backgroundColor: Colors.green));
            ctx.pop();
          }
          ...
          return Stack(children: [
            Form(key: _formKey, child: ListView(padding: const EdgeInsets.all(16), children: [
              ...
              Row(children: [
                ...
                const SizedBox(width: 12),
                ChoiceChip(label: const Text('Contado'), selected: _tipo == TipoVenta.CONTADO, onSelected: (_) => setState(() => _tipo = TipoVenta.CONTADO)),
                const SizedBox(width: 8),
                ChoiceChip(label: const Text('Crédito'), selected: _tipo == TipoVenta.CREDITO, onSelected: (_) => setState(() => _tipo = TipoVenta.CREDITO)),
              ...
              const Divider(),
              ListTile(contentPadding: EdgeInsets.zero, title: Text(_almNombre ?? 'Seleccionar almacén'), trailing: const Icon(Icons.warehouse), onTap: _pickAlmacen),
              ListTile(contentPadding: EdgeInsets.zero, title: Text(_cliNombre ?? 'Seleccionar cliente'), trailing: const Icon(Icons.person), onTap: _pickCliente),
              ...
              ..._lineas.asMap().entries.map((e) => ListTile(
                ...
                subtitle: Text('${e.value.cantidad} × ${e.value.precio.toStringAsFixed(4)}${e.value.descPct > 0 ? " (-${e.value.descPct}%)" : ""} = S/ ${e.value.importe.toStringAsFixed(2)}'),
...
class _TRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
      ...
      SizedBox(width: 90, child: Text('S/ ${value.toStringAsFixed(2)}', textAlign: TextAlign.right,
