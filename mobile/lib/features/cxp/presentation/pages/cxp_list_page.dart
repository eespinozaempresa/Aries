  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Builder(builder: (ctx) => Scaffold(
        appBar: AriesAppBar(
          title: const Text('Cuentas x Pagar'),
          actions: [
                  ...
                  onSelected: (v) {
                    switch (v) {
                      case 'todas':
                        ...
                        ctx.read<CxPBloc>().add(CxPLoad(reset: true, pendiente: null));
                        break;
                      case 'pendientes':
                        setState(() => _filtroPendiente = true);
                        ctx.read<CxPBloc>().add(CxPLoad(reset: true, pendiente: true));
                        break;
                      case 'canceladas':
                        setState(() => _filtroPendiente = false);
                        ctx.read<CxPBloc>().add(CxPLoad(reset: true, pendiente: false));
                        break;
                      case 'exportar':
            ...
            return ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                final vencida = cxp.fechaVencimiento != null &&
                  DateTime.tryParse(cxp.fechaVencimiento!)?.isBefore(FechaHoraUtil.ahora()) == true &&
                  cxp.pendiente;
                return ListTile(
                  onTap: () async {
