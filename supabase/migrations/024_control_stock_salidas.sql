-- ============================================================
-- Agrega el parámetro 'control_stock_salidas' (SI/NO, default NO)
-- y valida en registrar_movimiento que, cuando está en 'SI', las
-- operaciones de SALIDA y TRASLADO no dejen el stock en negativo.
-- ============================================================

-- 1. Nueva columna en parametros
ALTER TABLE parametros
  ADD COLUMN IF NOT EXISTS control_stock_salidas VARCHAR(2) NOT NULL DEFAULT 'NO'
  CHECK (control_stock_salidas IN ('SI', 'NO'));

-- 2. Actualizar registrar_movimiento: valida stock suficiente en SALIDA/TRASLADO
--    cuando control_stock_salidas = 'SI' para la empresa.
...
DECLARE
  ...
  v_control_stock VARCHAR(2);
BEGIN
  v_control_stock := COALESCE(
    (SELECT control_stock_salidas FROM parametros WHERE codigo_empresa = p_empresa),
    'NO'
  );
  ...
  LOOP
    ...
    IF v_control_stock = 'SI' AND p_tipo IN ('SALIDA', 'TRASLADO') AND v_stk_orig < v_qty THEN
      RAISE EXCEPTION 'STOCK_INSUFICIENTE: articulo % en almacen % (stock %, solicitado %)',
        v_art, p_alm_origen, v_stk_orig, v_qty
        USING ERRCODE = 'AR001';
    END IF;
